...
$(document).ready(function() {
  ...
  (function() {
    ...
    chrome.storage.sync.get("bgFrequency", function(options) {
      ...
      if (chrome.runtime.lastError) {
        imageSchedule = imageScheduleDefault;
      }
      else {
        imageSchedule = options.bgFrequency;
      }
      ...
      if (imageSchedule === "daily") {
        ...
        if (localStorage.getItem("lastImageDate") === currentDate) {
        ...
        if (localStorage.getItem("lastImageHour") === currentHour.toString()) {
          displayDataFromLocalStorage();
        }
        else {
          localStorage.setItem("lastImageHour", currentHour);
          // make a new API call    
          requestNewImage();
        }
      }
  ...
  function displayDataFromLocalStorage() {
    ...
    if(localStorage.getItem("imageURL") != null) {
      ...
      $(imageAttributionReference).hide();
      // make a new API call    
      requestNewImage();
    }

    if(localStorage.getItem("attribution") != null) {
      $(photographerReference).show();
